...
class Firebase
{
     ...
     */
    private function _getJsonPath($path, $queryString = '')
    {
        $url = $this->_baseURI;
        $path = ltrim($path, '/');
        $auth = '';
        if ($this->_token != '') {
            $auth = (($queryString == '') ? '?' : '&') . 'auth=' . $this->_token;
        }

        return $url . $path . '.json' . $queryString . $auth;
    }
    ...
    {
        try {
            $ch = $this->_getCurlHandler($path, 'GET', $queryString);
            $return = curl_exec($ch);
            curl_close($ch);
        } catch (Exception $e) {
            $return = null;
     ...
     */
    private function _getCurlHandler($path, $mode, $queryString = '')
    {
        $url = $this->_getJsonPath($path, $queryString);
        $ch = curl_init();
